(() => {
  ...
  const TURNSTILE_CONTAINER_ID = "admin-turnstile-widget";
  const TURNSTILE_TOKEN_KEY = "__adminTurnstileToken";
  const TURNSTILE_CONFIG_KEY = "__adminTurnstileConfig";
  const TURNSTILE_WIDGET_KEY = "__adminTurnstileWidgetId";
  const TURNSTILE_PATCH_KEY = "__adminTurnstilePatched";
  const TURNSTILE_SCRIPT_ID = "admin-turnstile-script";

  const POW_PROOF_KEY = "__adminPowProof";
  ...
  const isLoginRoute = () => {
    ...
    return hash.startsWith("#/sign-in") || hash.startsWith("#/sign-in-2");
  };
  ...
  const findLoginContainer = () => {
    return document.querySelector("form") || document.querySelector("#root");
  };
  ...
  const ensureTurnstileContainer = () => {
    ...
    container.className = "admin-turnstile";
  ...
  const ensurePowProof = async () => {
    const config = await loadConfig();
    if (!isPowEnabled(config) || !isLoginRoute()) {
      setPowProof(null);
      return null;
    }
  ...
  const shouldHandleUrl = (url) => typeof url === "string" && url.indexOf(LOGIN_PATH) !== -1;
  ...
  const patchNetwork = () => {
    ...
    XMLHttpRequest.prototype.open = function (method, url) {
      this.__adminTurnstileUrl = url;
      return originalOpen.apply(this, arguments);
    };
    XMLHttpRequest.prototype.send = function (body) {
      if (!shouldHandleUrl(this.__adminTurnstileUrl || "")) {
