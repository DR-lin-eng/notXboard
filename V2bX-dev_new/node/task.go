...
func (c *Controller) nodeInfoMonitor() (err error) {
	...
	// get user info
	userSync, err := c.apiClient.GetUserList()
	if err != nil {
		log.WithFields(log.Fields{
			"tag": c.tag,
			"err": err,
	...
	if newN != nil {
		c.info = newN
		// nodeInfo changed
		if userSync != nil && !userSync.NotModified {
			plan, planErr := buildUserApplyPlan(c.userList, userSync)
			if planErr != nil {
				log.WithFields(log.Fields{
					"tag": c.tag,
					"err": planErr,
				}).Error("Build reload user apply plan failed")
				return nil
			}
			c.userList = cloneUsers(plan.NextUsers)
			c.userSnapshotVersion = userSync.Version
		}
		c.traffic = make(map[string]int64)
		// Remove old node
	...
	plan, err := buildUserApplyPlan(c.userList, userSync)
	if err != nil {
		log.WithFields(log.Fields{
			"tag": c.tag,
			"err": err,
		}).Error("Build user apply plan failed")
		return nil
	}
	deleted, added := plan.Deleted, plan.Added
	if len(deleted) > 0 {
		// have deleted users
